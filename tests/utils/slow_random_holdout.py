...
def slow_random_holdout(test_size: float, random_state: int)->[Callable, str, Tuple[float, int]]:
...
def slow_random_holdouts(test_sizes: List[float], quantities: List[int], random_state: int = 42)->List[Tuple[Callable, str, List]]:
    ...
    if len(test_sizes) > 1:
        return [
            (
                *slow_random_holdout(test_sizes[0], random_state+i),
                slow_random_holdouts(test_sizes[1:], quantities[1:], random_state+i)
            ) for i in range(quantities[0])
        ]
    return [(*slow_random_holdout(test_sizes[0], random_state+i), None) for i in range(quantities[0])]
